...
CI = quantile(x = total_loss, probs = CDF_bounds, type = 8)
```

**We are 95% confident that the `r p`-quantile lies between `r dollar(unname(CI[1]))` and `r dollar(unname(CI[2]))`.** 
...
alpha = 0.05
CI_nonparametric = quantile(bootstrap_results_1$t[,1], p = c(alpha/2, 1-alpha/2), type = 8)
dollar(CI_nonparametric)
...
abline(v = CI_nonparametric[1], col = "red")
abline(v = CI_nonparametric[2], col = "red")
```
...
```{r}
generalized_pareto_CDF = function(x, mu, sigma, xi){
  z = (x-mu)/sigma
  Re(1 - as.complex(1 + xi * z)^(-1/xi))
}

generalized_pareto_PDF = function(x, mu, sigma, xi){
  z = (x-mu)/sigma
  Re(1 / sigma * as.complex(1 + xi * z)^(-1/(xi + 1)))
}
```
...
```{r}
mu_tilde = 0 # I picked this value without using Method-of-Moments
xi_tilde = 0.5 * (1 - (mean(total_loss) - mu_tilde)^2/var(total_loss))
xi_tilde

sigma_tilde = (mean(total_loss) - mu_tilde) * (1 - xi_tilde)
sigma_tilde

check_CDF(data = total_loss, alpha_for_DKW = 0.05, CDF_to_check = generalized_pareto_CDF, mu = mu_tilde, sigma = sigma_tilde, xi = xi_tilde) +
  xlim(0,1e5) +
  ylim(0.5, 1)
```

Is the log-likelihood stable at these method-of-moments estimates?

```{r}
get_log_likelihood_for_gp_for_optim = function(params){
  get_log_likelihood(PDF = generalized_pareto_PDF, # taken from global env.
                   data = total_loss, # taken from global env.
                   mu = mu_tilde, # taken from global env.
                   sigma = params["sigma_hat"], 
                   xi = params["xi_hat"]
  )
}
# params = c(sigma_tilde = 1e4, xi_tilde = 0.7)
# get_log_likelihood(PDF = generalized_pareto_PDF, 
#                    data = total_loss, 
#                    mu = mu_tilde, 
#                    sigma = params["sigma_tilde"], 
#                    xi = params["xi_tilde"]
#   )

optim(par = c(sigma_hat = sigma_tilde, xi_hat = xi_tilde), 
      fn = get_log_likelihood_for_gp_for_optim,
      method = "L-BFGS-B",
      lower = c(10, 0),
      upper = c(5e6, 5e6),
      control = list(parscale = c(1,1e-4),
                     fnscale = -1)) # turn it into a maximization problem
...
# Select the best parametric model

```{r}
get_AIC = function(model_names = NULL, k, max_log_likelihood){
  AIC_comparison = 2*k - 2 * max_log_likelihood
  names(AIC_comparison) = model_names
  AIC_comparison # lower is better
}

get_AIC(k = c(3, 3), max_log_likelihood = c(-20918.16, -20342.89))
```

# Bootstrapping the p-Quantile--A Parametric Approach

```{r}
# Reduce the number of replications to speed up computation time.
R = 1e4
# This calculates the bootstrap difference between the estimated p-quantile from a bootstrap sample of the fitted distribution and the point-estimate of the p-quantile from the original data. 
delta_star = function(data, i, p, theta_hat){
  quantile(data[i], p, type = 8, names = FALSE) - unname(theta_hat)
}

bootstrap_results_2 = boot(
  data = rgp(n = num_days, mu = 0, sigma = 1e4, xi = 0.7), 
  statistic = delta_star, 
  p = p, 
  theta_hat = p_quantile,
  R = R, 
  sim = "ordinary", 
  parallel = "snow", 
  ncpus = cpus_on_machine
)
...
quantiles_of_delta_star = quantile(bootstrap_results_2$t[,1], p = c(alpha/2, 1-alpha/2), type = 8)
CI_parametric = c(p_quantile - quantiles_of_delta_star[2],
                  p_quantile - quantiles_of_delta_star[1])
names(CI_parametric) = c(paste0((alpha/2) * 100, "%"),
                         paste0((1 - alpha/2) * 100, "%")
)
CI_parametric

hist(bootstrap_results_2$t[,1] + p_quantile, nclass = 30, xlab = "Bootstrap Error from Point-Estimate", main = paste0("Bootstrap Estimate of ", p, "-Quantile of Total Loss\nR = ", R))
abline(v = CI_parametric[1], col = "red")
abline(v = CI_parametric[2], col = "red")
```
...
7. [Joint Distribution of Order Statistics](https://en.wikipedia.org/wiki/Order_statistic#The_joint_distribution_of_the_order_statistics_of_an_absolutely_continuous_distribution)