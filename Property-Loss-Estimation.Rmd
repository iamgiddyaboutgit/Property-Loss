...
One weakness of the Dvoretzky–Kiefer–Wolfowitz bands is that their coverage probability is higher nearer the median of the distribution but lower near the tails. However, these bands very easily capture where the true CDF may lie.
...
We can then use our preferred method of constructing a CI for a binomial success probability to get a point-wise CI for \(F \left( x \right)\). We will try to construct an exact Clopper-Pearson CI. Note that this interval will be conservative.

```{r}
# This function constructs a lower bound for a pointwise CI for the true CDF at a given value of x and then subtracts 0.80 from that for optimization purposes. A Clopper-Pearson CI is used.
get.cdf_pointwise_conf_int_LB = function(x, n, my_ecdf){
  binom.test(x = floor(n * my_ecdf(x)) + 1,
           n = num_claims,
           p = 0.8)$conf.int[1] - 0.80
}
# This function constructs an upper bound for a pointwise CI for the true CDF at a given value of x and then subtracts 0.80 from that for optimization purposes. A Clopper-Pearson CI is used.
get.cdf_pointwise_conf_int_UB = function(x, n, my_ecdf){
  binom.test(x = floor(n * my_ecdf(x)) + 1,
           n = num_claims,
           p = 0.8)$conf.int[2] - 0.80
}

# Find quantiles by inverting the LB and UB for the CDF.
LB = uniroot(get.cdf_pointwise_conf_int_UB, n = num_claims, my_ecdf = total_loss_ecdf, interval = c(3e4,4e4), extendInt = "upX")$root

UB = uniroot(get.cdf_pointwise_conf_int_LB, n = num_claims, my_ecdf = total_loss_ecdf, interval = c(3e4,4e4), extendInt = "upX")$root
```

We are 95% confident that the 80th percentile lies between `r dollar(LB)` and `r dollar(UB)`. 
