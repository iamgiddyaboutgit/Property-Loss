...
# lucky guess? 
check_CDF(data = total_loss, alpha_for_DKW = 0.1, CDF_to_check = generalized_pareto_CDF, mu = -1e4, sigma = 1.7e4, xi = 0.49) +
  xlim(0,1e5) +
  ylim(0.5, 1)
```
...
```{r}
get_log_likelihood(PDF = generalized_pareto_PDF, data = total_loss, mu = 0, sigma = 1e4, xi = 0.7)
# -41935.12
# Check above for stability
mu_hat = -1e4
sigma_hat = 1.7e4
xi_hat = 0.49
get_log_likelihood(PDF = generalized_pareto_PDF, data = total_loss, mu = mu_hat, sigma = sigma_hat, xi = xi_hat)


check_CDF(data = total_loss, alpha_for_DKW = 0.05, CDF_to_check = generalized_pareto_CDF, mu = mu_hat, sigma = sigma_hat, xi = xi_hat) 
```
...
```{r}
mu_tilde = 0
xi_tilde = 0.5 * (1 - (mean(total_loss[total_loss > 0]) - mu_tilde)^2/var(total_loss[total_loss > 0]))
xi_tilde

sigma_tilde = (mean(total_loss[total_loss > 0]) - mu_tilde) * (1 - xi_tilde)
sigma_tilde

get_log_likelihood(PDF = generalized_pareto_PDF, data = total_loss[total_loss > 0], mu = mu_tilde, sigma = sigma_tilde, xi = xi_tilde)
